...
for cat in cat_list:
  df = pd.read_csv(f"./SafetyHeadAttribution/exp_data/unused_beaver/{cat}_CoT.csv")
  with open(f"./SafetyHeadAttribution/exp_data/unused_beaver/llama_{cat}_vanilla_answers.json", "w+") as fp:
    for _,i in df.iterrows():
      inputs = tokenizer([i["input"]], truncation=True, padding = True, return_tensors="pt").to("cuda:0")

      generate_ids = model.generate(inputs.input_ids, attention_mask = inputs.attention_mask, max_new_tokens =256, pad_token_id = tokenizer.pad_token_id)
