...
for cat in cat_list:
    ...
    for i in range(5):
        top_heads= {}
        rand_head = np.random.choice(31, 2)
        while np.any(np.all(taken == rand_head, 1)):
            rand_head = np.random.choice(31, 2)
        top_heads[str(rand_head[0]) + "-" + str(rand_head[1])] = i
        mask_n = 1
    #for mask_n in range(1, 6):
        with open(f"./SafetyHeadAttribution/exp_res/{cat}-mistral-search5-scale1e-5/dataset_ships_ablated_random_head{list(top_heads.keys())[0]}.jsonl",
                  "w+") as fp:
            for idx, data in tqdm.tqdm(test.data.iterrows()):
               generated_text = test.ships_generate(input_text=data['input'],
                                                    top_ships=top_heads,
                                                    top_k=1,
                                                    mask_num=mask_n,
                                                    max_new_tokens=256)
               output = {data['input']: generated_text[len(data['input']):]}
